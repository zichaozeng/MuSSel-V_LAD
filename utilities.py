"""Utility scripts for visual place recognition experiments.

This module provides various utility classes and functions including:
- Dataset abstractions for VPR benchmarks
- Type conversion utilities (NumPy, PIL, tensors)
- Feature extraction wrappers for vision models
- Visualisation and plotting functions
- FAISS-based retrieval utilities

Note: This file contains auxiliary utilities and may not be actively used
in the main MuSSel-V pipeline. The core utilities are in MuSSel/utils.py.
"""
...
class CustomDataset:
    """
    Abstract base class for custom VPR datasets.
    
    Provides a standard interface for dataset implementations including:
    - Image path management
    - Ground truth positive retrieval
    - Relative path extraction for caching
    
    Subclasses should implement:
    - __getitem__: Retrieve image or features at index
    - Initialise: database_num, queries_num, soft_positives_per_query
    """
    def __init__(self) -> None:
        # Required properties
        self.database_num = None              # Number of database items
        self.queries_num = None               # Number of queries
        self.soft_positives_per_query = None  # Soft positives per query
