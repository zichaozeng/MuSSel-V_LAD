"""Laurel Caverns dataset loader for visual place recognition.

This module provides a PyTorch dataset class for the Laurel Caverns dataset,
which consists of images captured in an underground cave environment.

The dataset includes:
- Database images (reference set)
- Query images
- Ground truth pose information
- Positive correspondence generation based on spatial proximity

Usage:
    dataset = Laurel(args, datasets_folder='./workspace', split='train')
"""
...
class Laurel(CustomDataset):
    """
    PyTorch dataset class for Laurel Caverns underground environment.
    
    This dataset contains images from an underground cave system with challenging
    lighting conditions and repeating structures. It's designed for testing VPR
    algorithms in extreme indoor environments.
    
    Args:
        args: Configuration arguments (may contain dataset-specific settings).
        datasets_folder (str): Root directory containing dataset folders.
        dataset_name (str): Dataset identifier (default: 'laurel').
    ...
    """
    def __init__(self,args,datasets_folder,dataset_name="laurel",split="train",use_ang_positives=False,dist_thresh = 8,ang_thresh=20,use_mixVPR=False,use_SAM=False):
        super().__init__()
